...
void particleFilter(int * I, int IszX, int IszY, int Nfr, int * seed, int Nparticles){
	...
	double * CDF = (double *)mgpu::cudaMallocHost(sizeof(double)*Nparticles);
	...
	double * u = (double *)mgpu::cudaMallocHost(sizeof(double)*Nparticles);
	double * u_GPU;
	
	//CUDA memory allocation
    arrayX_GPU = (double *) mgpu::cudaMalloc(sizeof(double)*Nparticles);
    arrayY_GPU = (double *) mgpu::cudaMalloc(sizeof(double)*Nparticles);
    xj_GPU = (double *) mgpu::cudaMalloc(sizeof(double)*Nparticles);
    yj_GPU = (double *) mgpu::cudaMalloc(sizeof(double)*Nparticles);
    CDF_GPU = (double *) mgpu::cudaMalloc(sizeof(double)*Nparticles);
    u_GPU = (double *) mgpu::cudaMalloc(sizeof(double)*Nparticles);
	...
	for(k = 1; k < Nfr; k++){
		...
		mgpu::cudaMemcpy(arrayX_GPU, arrayX, sizeof(double)*Nparticles, cudaMemcpyHostToDevice);
		mgpu::cudaMemcpy(arrayY_GPU, arrayY, sizeof(double)*Nparticles, cudaMemcpyHostToDevice);
		...
		mgpu::cudaMemcpy(CDF_GPU, CDF, sizeof(double)*Nparticles, cudaMemcpyHostToDevice);
		mgpu::cudaMemcpy(u_GPU, u, sizeof(double)*Nparticles, cudaMemcpyHostToDevice);
        ...
        mgpu::cudaLaunchKernel(conf, "/opt/custom/ptx/particle_filter_naive.ptx", "kernel", arrayX_GPU, arrayY_GPU,
                               CDF_GPU, u_GPU, xj_GPU, yj_GPU, Nparticles);
//                cudaThreadSynchronize();
        mgpu::cudaStreamSynchronize(nullptr);
                long long start_copy_back = get_time();
		//CUDA memory copying back from GPU to CPU memory
		mgpu::cudaMemcpy(yj, yj_GPU, sizeof(double)*Nparticles, cudaMemcpyDeviceToHost);
		mgpu::cudaMemcpy(xj, xj_GPU, sizeof(double)*Nparticles, cudaMemcpyDeviceToHost);
