...
__device__ void kernel_V(double * arrayX, double * arrayY, double * CDF, double * u, double * xj, double * yj, int Nparticles, uint3 blockIDX);

extern "C" __global__ void

kernel_V1(double * arrayX, double * arrayY, double * CDF, double * u, double * xj, double * yj, int Nparticles) {
...
#pragma unroll
        for (int i = 0; i < GET_ITERS(flags); i++) {
            ...
            __syncthreads();
            kernel_V(arrayX, arrayY, CDF, u, xj, yj, Nparticles, blockIDX);
        }
        if(IS_PRIMARY_WORKER) {
            if(IS_LEAD_THREAD) {
                ...
                if(start_block > 0) {
                    ...
                    kernel_V1<<<start_block, blockDim, 0, tmp>>>(arrayX, arrayY, CDF, u, xj, yj, Nparticles);
