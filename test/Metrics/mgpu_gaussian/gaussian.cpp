        ...
        if (argv[i][0]=='-') {// flag
            ...
            switch (flag) {
                case 's': // platform
                    ...
                    a = (float *) mgpu::cudaMallocHost(Size * Size * sizeof(float));
                    create_matrix(a, Size);

                    b = (float *) mgpu::cudaMallocHost(Size * sizeof(float));
                    for (j =0; j< Size; j++)
                        b[j]=1.0;

                    m = (float *) mgpu::cudaMallocHost(Size * Size * sizeof(float));
                    break;
                case 'f': // platform
                    i++;
...
void ForwardSub()
{
    ...
    m_cuda = static_cast<float*>(mgpu::cudaMalloc(Size * Size * sizeof(float)));

    a_cuda = static_cast<float*>(mgpu::cudaMalloc(Size * Size * sizeof(float)));

    b_cuda = static_cast<float*>(mgpu::cudaMalloc(Size * sizeof(float)));

    // copy memory to GPU
    mgpu::cudaMemcpy(m_cuda, m, Size * Size * sizeof(float),cudaMemcpyHostToDevice );
    mgpu::cudaMemcpy(a_cuda, a, Size * Size * sizeof(float),cudaMemcpyHostToDevice );
    mgpu::cudaMemcpy(b_cuda, b, Size * sizeof(float),cudaMemcpyHostToDevice );
    ...
    gettimeofday(&time_start, NULL);
    for (t=0; t<(Size-1); t++) {
        mgpu::cudaLaunchKernel({dimGrid, dimBlock}, "/opt/custom/ptx/gaussian.ptx", "Fan1", m_cuda, a_cuda, Size, t);
        mgpu::cudaStreamSynchronize(0);
        mgpu::cudaLaunchKernel({dimGridXY, dimBlockXY}, "/opt/custom/ptx/gaussian.ptx", "Fan2", m_cuda, a_cuda, b_cuda, Size, Size-t, t);
        mgpu::cudaStreamSynchronize(0);
    }
    // end timing kernels
    struct timeval time_end;
    ...
    mgpu::cudaMemcpy(m, m_cuda, Size * Size * sizeof(float),cudaMemcpyDeviceToHost );
    mgpu::cudaMemcpy(a, a_cuda, Size * Size * sizeof(float),cudaMemcpyDeviceToHost );
    mgpu::cudaMemcpy(b, b_cuda, Size * sizeof(float),cudaMemcpyDeviceToHost );
    mgpu::cudaFree(m_cuda);
    mgpu::cudaFree(a_cuda);
    mgpu::cudaFree(b_cuda);
}
